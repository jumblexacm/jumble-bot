...
4. Create your IAM [user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id.html#id_which-to-choose)
    ...
    - Attach existing policies directly: `assume-admin-policy`
...
2. Create an IAM engineer role
    ...
    - An AWS account: `This account`
    - Options: `Require MFA`
    - Add permissions: `lambda-policy`
    - Name: `engineer`

3. Create an IAM "assume engineer role" policy
    - Use most of the same steps from `assume-admin-policy`
...
5. For each user (including the first IAM user from earlier), add the `assume-engineer-policy`
